            ...
            $match: {
              $and: [
                ...
                },
                {
                  books: {
                    $elemMatch: {
                      title: { $eq: req.query.bookName.toLowerCase() },
                    },
                  },
                },
              ],
...
router.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    if (foundUser.books.length === 0) {
      ...
      const data = await oneBook(bookId).catch((error) => {
        throw createError(500, error.message);
      }); // Need to improve to throw error ****//
      ...
      res.json({
        newBook: newBook,
        message: `${newBook.title} is added to your collection`,
      });
...
router.delete(
  ...
  wrapAsync(async (req, res) => {
    ...
    await foundBook.remove().catch((error) => {
      throw createError(400, "Something went wrong");
    });
    res.json({
      removedBook: foundBook,
      message: `${foundBook.title} is removed from your collection`,
    });
  })
);
